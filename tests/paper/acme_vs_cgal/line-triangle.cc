...
int main()
{
  std::cout << "CGAL VS ACME: LINE-TRIANGLE" << std::endl
            << std::endl;
  ...
  for (unsigned int i = 0; i < n_samples; ++i)
  {
    ...
    Line entity2_cgal(Point(p4x, p4y, p4z), Point(p5x, p5y, p5z));
    // Start chronometer
    tictoc_cgal.tic();
    ...
    line *entity2_acme = new line(point(p4x, p4y, p4z), point(p5x, p5y, p5z));
    // Start chronometer
    tictoc_acme.tic();
