...
int main()
{
  std::cout << "CGAL VS ACME: LINE-LINE" << std::endl
            << std::endl;
  ...
  for (unsigned int i = 0; i < n_samples; ++i)
  {
    ...
    Line entity1_cgal(Point(p1x, p1y, p1z), Point(p2x, p2y, p2z));
    Line entity2_cgal(Point(p3x, p3y, p3z), Point(p4x, p4y, p4z));
    // Start chronometer
    tictoc_cgal.tic();
    ...
    line *entity1_acme = new line(point(p1x, p1y, p1z), point(p2x, p2y, p2z));
    line *entity2_acme = new line(point(p3x, p3y, p3z), point(p4x, p4y, p4z));
    // Start chronometer
    tictoc_acme.tic();
