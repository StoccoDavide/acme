...
int main()
{
  std::cout << "CGAL VS ACME: SEGMENT-SEGMENT" << std::endl
            << std::endl;
  ...
  for (unsigned int i = 0; i < n_samples; ++i)
  {
    ...
    Segment entity1_cgal(Point(p1x, p1y, p1z), Point(p2x, p2y, p2z));
    Segment entity2_cgal(Point(p3x, p3y, p3z), Point(p4x, p4y, p4z));
    // Start chronometer
    tictoc_cgal.tic();
    ...
    segment *entity1_acme = new segment(point(p1x, p1y, p1z), point(p2x, p2y, p2z));
    segment *entity2_acme = new segment(point(p3x, p3y, p3z), point(p4x, p4y, p4z));
    // Start chronometer
    tictoc_acme.tic();
