...
namespace acme
{
   ...
   template <typename T, std::size_t M, std::size_t N>
   matrix<T, M, N> &matrix<T, M, N>::operator+=(const matrix<T, M, N> &_matrix)
   {
      for (std::size_t i = 0; i < size(); ++i)
      {
   ...
   template <typename T, std::size_t M, std::size_t N>
   void matrix<T, M, N>::swap(const unsigned int &x1, const unsigned int &y1,
   ...
   template <typename T>
   void eigenvalues(const matrix<T, 2, 2> &matrix, T &eigenvalue1, T &eigenvalue2)
   {
   ...
   template <typename T>
   void eigenvector(const matrix<T, 2, 2> &matrix,
