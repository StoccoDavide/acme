...
namespace acme
{
  ...
  template <typename T, std::size_t D>
  inline line<T, D> mirror(const line<T, D> &line, const acme::line<T, D> &mirror_axis)
  {
    acme::line<T, D> _line;
  ...
  inline polygon<T, D> make_polygon(const triangle<T, D> &triangle)
  {
    ...
    polygon.reserve(acme::triangle<T, D>::PointCount);
  ...
  inline polygon<T, D> make_polygon(const quadix<T, D> &quadix)
  {
    ...
    polygon.reserve(acme::quadix<T, D>::PointCount);
