...
namespace acme
{
   #define INSTANTIATE_ACME(T,InputIterator2d,InputIterator3d,OutputIterator2d,OutputIterator3d)\
      ...
      template void create_right_triangle<T>(const acme::point2d<T>& p1, const acme::point2d<T>& p2, acme::point2d<T>& c1, acme::point2d<T>& c2);\
      ...
      template bool is_right_triangle<T>(const acme::triangle<T,2>& triangle);\
      template bool is_right_triangle<T>(const acme::triangle<T,3>& triangle);\
      ...
      template line<T,2> mirror<T>(const line<T,2>& line, const acme::line<T,2>& mirror_axis);\
      ...
      template line<T,3> mirror<T>(const line<T,3>& line, const acme::line<T,3>& mirror_axis);\
   ...
   #define INSTANTIATE_ACME_ND(T, D, OutputIterator)\
      ...
      template pointnd<T,D> closest_point_on_segment_from_point<T,D>(const segment<T,D>& segment, const acme::segment<T,D>::PointType& point);\
      template pointnd<T,D> closest_point_on_line_from_point<T,D>(const line<T,D>& segment, const acme::line<T,D>::PointType& point);\
      template pointnd<T,D> closest_point_on_plane_from_point<T,D>(const plane<T,D>& plane, const acme::plane<T,D>::PointType& point);\
      template pointnd<T,D> closest_point_on_box_from_point<T,D>(const box<T,D>& box, const acme::box<T,D>::PointType& point);\
      ...
      template line<T,D> mirror<T,D>(const line<T,D>& line, const acme::line<T,D>& mirror_axis);\
   ...
   #define INSTANTIATE_ACME_ALGORITHMS(T,K)                                                                                                \
   ...
   typedef acme::point2d<float>*  flt_pnt_2d;
   typedef acme::point2d<double>* dbl_pnt_2d;

   typedef acme::point3d<float>*  flt_pnt_3d;
   typedef acme::point3d<double>* dbl_pnt_3d;

   INSTANTIATE_ACME( float, flt_pnt_2d, flt_pnt_3d, flt_pnt_2d, flt_pnt_3d)
   INSTANTIATE_ACME(double, dbl_pnt_2d, dbl_pnt_3d, dbl_pnt_2d, dbl_pnt_3d)

   INSTANTIATE_ACME_MATH(float)
   INSTANTIATE_ACME_MATH(double)

   INSTANTIATE_ACME_UTILITIES_1(float)
   INSTANTIATE_ACME_UTILITIES_1(double)

   INSTANTIATE_ACME_UTILITIES_2(float,2)
   INSTANTIATE_ACME_UTILITIES_2(double,2)

   INSTANTIATE_ACME_UTILITIES_2(float,3)
   INSTANTIATE_ACME_UTILITIES_2(double,3)
