...
namespace acme
{
   ...
   template <typename T>
   int orientation(const point2d<T>& point1,
   ...
   template <typename T>
   int orientation(const line<T,2>&  line,
   ...
   template <typename T>
   int orientation(const segment<T,2>& segment,
                          const point2d<T>&   point)
   {
   ...
   template <typename T>
   int orientation(const point3d<T>& point1,
   ...
   template <typename T>
   int orientation(const triangle<T,3>& triangle,
   ...
   template <typename T>
   bool robust_collinear(const point2d<T>& point1, const point2d<T>& point2, const point2d<T>& point3, const T& epsilon)
   {
   ...
   template <typename T>
   bool robust_collinear(const point3d<T>& point1, const point3d<T>& point2, const point3d<T>& point3, const T& epsilon)
   {
   ...
   template <typename T>
   bool robust_collinear(const line<T,2>& line, const point2d<T>& point, const T& epsilon)
   {
      return robust_collinear(line[0],line[1],point,epsilon);
   }

   template <typename T>
   bool robust_collinear(const line<T,3>& line, const point3d<T>& point, const T& epsilon)
   {
      return robust_collinear(line[0],line[1],point,epsilon);
   }
   ...
   template <typename T>
   bool robust_coplanar(const point3d<T> point1,
                               const point3d<T> point2,
                               const point3d<T> point3,
                               const point3d<T> point4,
                               const T& epsilon)
   ...
   template <typename T>
   bool cocircular(const T& x1, const T& y1,
                          const T& x2, const T& y2,
                          const T& x3, const T& y3,
   ...
   template <typename T>
   bool cocircular(const point2d<T>& point1,
                          const point2d<T>& point2,
                          const point2d<T>& point3,
                          const point2d<T>& point4,
                          const T& epsilon)
   ...
   template <typename T>
   bool cocircular(const triangle<T,2>& triangle, const point2d<T>& point, const T& epsilon)
   {
      return cocircular(triangle[0],triangle[1],triangle[2],point,epsilon);
   }

   template <typename T>
   bool cocircular(const circle<T>& circle, const point2d<T>& point, const T& epsilon)
   {
      return is_equal(distance(point.x,point.y,circle.x,circle.y),circle.radius,epsilon);
   }

   template <typename T>
   bool is_skinny_triangle(const T& x1, const T& y1,
                                  const T& x2, const T& y2,
                                  const T& x3, const T& y3)
   {
      /*
   ...
   template <typename T>
   bool is_skinny_triangle(const point2d<T>& point1,
                                  const point2d<T>& point2,
                                  const point2d<T>& point3)
   {
   ...
   template <typename T>
   bool intersect(const point2d<T>& point1,const point2d<T>& point2,
   ...
   template <typename T>
   bool intersect(const segment<T,3>& segment1, const segment<T,3>& segment2, const T& fuzzy)
   {
   ...
   template <typename T>
   bool intersect(const segment<T,3>& segment, const line<T,3>& line, const T& fuzzy)
   {
      return intersect
             (
               segment,
   ...
   template <typename T, typename OutputIterator>
   void intersection_point(const ray<T,2>& ray,
                                  const circle<T>& circle,
                                  OutputIterator out)
   ...
   template <typename T, typename OutputIterator>
   void intersection_point(const ray<T,3>& ray,
                                  const sphere<T>& sphere,
                                  OutputIterator out)
   {
   ...
   {
      acme::segment<T,2> tmp_segment = translate(-circle.x, -circle.y, segment);

      const T rsqr  = acme::sqr(circle.radius);
      const T drsqr = lay_distance(tmp_segment);
      const T dsqr  = acme::sqr(tmp_segment[0].x * tmp_segment[1].y - tmp_segment[1].x * tmp_segment[0].y);
   ...
   template <typename T>
   segment<T,2> edge(const rectangle<T>& rectangle, const std::size_t& edge_index)
   {
      switch(edge_index)
      {
   ...
   {
      acme::triangle<T,2> tri = triangle;
      ...
      const acme::triangle<T,2> triangle1 = create_triangle(edge(tri, 0), angle1, angle2);
      const acme::triangle<T,2> triangle2 = create_triangle(edge(tri, 1), angle2, angle3);
      const acme::triangle<T,2> triangle3 = create_triangle(edge(tri, 2), angle3, angle1);
   ...
   triangle<T,2> create_anticevian_triangle(const triangle<T,2>& triangle, const point2d<T>& point)
   {
      ...
      {
         typedef const acme::line<T,2> line2d_t;

         line2d_t opp_edge    = acme::make_line(opposing_edge(triangle,i));
         line2d_t cevian_edge = acme::make_line(triangle[i],point);
         line2d_t orthic_edge = acme::make_line
                                        (
                                          triangle[i],
                                          acme::closest_point_on_line_from_point(opp_edge,triangle[i])
                                        );

         //if (!robust_collinear(orthic_edge,point))
         {
            point_list[i] = acme::intersection_point(cevian_edge, make_line(orthic_edge[i], acme::mirror(point,opp_edge)));
         }
         //else
   ...
   {
      const point2d<T> p = make_point(acme::inscribed_circle(triangle));
   ...
   {
      acme::triangle<T,2> triangle_ = create_excentral_triangle(triangle);
   ...
   {
      acme::triangle<T,3> triangle_ = create_excentral_triangle(triangle);
   ...
   triangle<T,2> create_circumcevian_triangle(const triangle<T,2>& triangle, const point2d<T>& point)
   {
      ...
      acme::triangle<T,2> triangle_ = degenerate_triangle2d<T>();
   ...
   {
      const T distance = acme::distance(p1,p2);

      const acme::point2d <T> mid = acme::segment_mid_point(p1,p2);
      const acme::vector2d<T>   v = acme::normalize(acme::perpendicular(p1 - p2)) * (distance / T(2.0));
   ...
   template <typename T>
   bool trilateration(const T& c0x, const T& c0y, const T& c0r,
                                             ...
                                             std::vector<line<T,2> >& lines)
   {
      ...
      else
      {
         ...
         acme::point2d<T> c0pnt0;
         acme::point2d<T> c0pnt1;
         acme::point2d<T> c1pnt0;
         acme::point2d<T> c1pnt1;

         acme::circle_tangent_points(circle0, i, c0pnt0, c0pnt1);
         acme::circle_tangent_points(circle1, i, c1pnt0, c1pnt1);
                                          ...
                                          std::vector< line<T,2> >& lines)
   {
      ...
      else
      {
         ...
         acme::circle_tangent_points(circle0, p, c0pnt0, c0pnt1);
         acme::circle_tangent_points(circle1, p, c1pnt0, c1pnt1);
   ...
   template <typename T>
   point2d<T> closest_point_on_quadix_from_point(const quadix<T,2>& quadix,
                                                        const point2d<T>&  point)
   {
   ...
   template <typename T>
   point2d<T> centroid(const point2d<T>& point1,
                              const point2d<T>& point2,
                              const point2d<T>& point3,
                              const point2d<T>& point4)
   {
   ...
   void remove_consecutive_collinear_points(const InputIterator begin, const InputIterator end,
                                                   OutputIterator out)
   {
   ...
   template <typename T>
   bool is_right_triangle(const acme::triangle<T,2>& triangle)
   {
      const T a = acme::lay_distance(acme::edge(triangle,0));
      const T b = acme::lay_distance(acme::edge(triangle,1));
      const T c = acme::lay_distance(acme::edge(triangle,2));

      return acme::is_equal(a + b,c) || acme::is_equal(a + c,b) || acme::is_equal(b + c,a);
   }

   template <typename T>
   bool is_right_triangle(const acme::triangle<T,3>& triangle)
   {
      const T a = acme::lay_distance(acme::edge(triangle,0));
      const T b = acme::lay_distance(acme::edge(triangle,1));
      const T c = acme::lay_distance(acme::edge(triangle,2));
   ...
   {
      acme::segment<T,2> segment_;

      for (std::size_t i = 0; i < acme::segment<T,2>::PointCount; ++i)
      {
   ...
   template <typename T>
   line<T,2> mirror(const line<T,2>& line, const acme::line<T,2>& mirror_axis)
   {
      acme::line<T,2> line_;

      for (std::size_t i = 0; i < acme::line<T,2>::PointCount; ++i)
      {
   ...
   {
      acme::rectangle<T> rectangle_;

      for (std::size_t i = 0; i < acme::rectangle<T>::PointCount; ++i)
      {
   ...
   {
      acme::triangle<T,2> triangle_;

      for (std::size_t i = 0; i < acme::triangle<T,2>::PointCount; ++i)
      {
   ...
   {
      acme::quadix<T,2> quadix_;

      for (std::size_t i = 0; i < acme::quadix<T,2>::PointCount; ++i)
      {
   ...
   {
      acme::segment<T,3> segment_;

      for (std::size_t i = 0; i < acme::segment<T,3>::PointCount; ++i)
      {
         segment_[i] = mirror(segment[i],mirror_axis);
      }
   ...
   template <typename T>
   line<T,3> mirror(const line<T,3>& line, const acme::line<T,3>& mirror_axis)
   {
      acme::line<T,3> line_;

      for (std::size_t i = 0; i < acme::line<T,3>::PointCount; ++i)
      {
         line_[i] = mirror(line[i],mirror_axis);
      }
   ...
   box<T,3> mirror(const box<T,3>& box, const line<T,3>& mirror_axis)
   {
      ...
      for (std::size_t i = 0; i < acme::box<T,3>::PointCount; ++i)
      {
   ...
   {
      acme::triangle<T,3> triangle_;

      for (std::size_t i = 0; i < acme::triangle<T,3>::PointCount; ++i)
      {
         triangle_[i] = mirror(triangle[i],mirror_axis);
      }
   ...
   {
      acme::quadix<T,3> quadix_;

      for (std::size_t i = 0; i < acme::quadix<T,3>::PointCount; ++i)
      {
         quadix_[i] = mirror(quadix[i],mirror_axis);
      }
   ...
   template <typename T>
   point3d<T> mirror(const point3d<T>& point, const plane<T,3>& plane)
   {
      return point + T(2.0) * closest_point_on_plane_from_point(plane,point) - point;
   }

   template <typename T>
   segment<T,3> mirror(const segment<T,3>& segment, const plane<T,3>& plane)
   {
      acme::segment<T,3> segment_;
   ...
   template <typename T>
   line<T,3> mirror(const line<T,3>& line, const plane<T,3>& plane)
   {
      acme::line<T,3> line_;
   ...
   template <typename T>
   box<T,3> mirror(const box<T,3>& box, const plane<T,3>& plane)
   {
      acme::box<T,3> box_;
   ...
   template <typename T>
   triangle<T,3> mirror(const triangle<T,3>& triangle, const plane<T,3>& plane)
   {
      acme::triangle<T,3> triangle_;
   ...
   template <typename T>
   quadix<T,3> mirror(const quadix<T,3>& quadix, const plane<T,3>& plane)
   {
      acme::quadix<T,3> quadix_;
   ...
   template <typename T>
   sphere<T> mirror(const sphere<T>& sphere, const plane<T,3>& plane)
   {
   ...
   template <typename T>
   sphere<T> nonsymmetric_mirror(const sphere<T>& sphere, const T& ratio, const plane<T,3>& plane)
   {
      point3d<T> center = make_point(sphere);
   ...
   template <typename T>
   T lay_distance(const point2d<T>& point, const quadix<T,2>& quadix)
   {
   ...
   template <typename T>
   T span_length(const rectangle<T>& rect)
   {
   ...
   {
      acme::point2d<T> point_ = closest_point_on_line_from_point(axis,point);
   ...
   {
      acme::point3d<T> point_ = closest_point_on_line_from_point(axis,point);
      return make_segment(point_,point_);
   }
   ...
   {
      acme::segment<T,2> segment_ = segment;
   ...
   {
      acme::segment<T,3> segment_ = segment;
   ...
   template <typename T>
   bool clip(const T&  x1, const T&  y1,
                    const T&  x2, const T&  y2,
                    const T&  x3, const T&  y3,
                    const T&  x4, const T&  y4,
                          T& cx1,       T& cy1,
   ...
   template <typename T>
   bool clip(const T&  x1, const T&  y1, const T&  z1,
                    const T&  x2, const T&  y2, const T&  z2,
                    const T&  x3, const T&  y3, const T&  z3,
                    const T&  x4, const T&  y4, const T&  z4,
                          T& cx1,       T& cy1,       T& cz1,
   ...
   template <typename T>
   void rotate(const T& rx, const T& ry, const T& rz,
   ...
   template <typename T>
   point3d<T> rotate(const T& rx, const T& ry, const T& rz, const point3d<T>& point)
   {
      point3d<T> point_;
   ...
   template <typename T>
   point3d<T> rotate(const T& rx, const T& ry, const T& rz, const point3d<T>& point, const point3d<T>& opoint)
   {
      point3d<T> point_;
   ...
   template <typename T>
   segment<T,3> rotate(const T& rx, const T& ry, const T& rz, const segment<T,3>& segment)
   {
      acme::segment<T,3> segment_;
   ...
   template <typename T>
   segment<T,3> rotate(const T& rx, const T& ry, const T& rz, const segment<T,3>& segment, const point3d<T>& opoint)
   {
      acme::segment<T,3> segment_;
   ...
   template <typename T>
   triangle<T,3> rotate(const T& rx, const T& ry, const T& rz, const triangle<T,3>& triangle)
   {
      acme::triangle<T,3> triangle_;
   ...
   template <typename T>
   triangle<T,3> rotate(const T& rx, const T& ry, const T& rz, const triangle<T,3>& triangle, const point3d<T>& opoint)
   {
      acme::triangle<T,3> triangle_;
   ...
   template <typename T>
   quadix<T,3> rotate(const T& rx, const T& ry, const T& rz, const quadix<T,3>& quadix)
   {
      acme::quadix<T,3> quadix_;
   ...
   template <typename T>
   quadix<T,3> rotate(const T& rx, const T& ry, const T& rz, const quadix<T,3>& quadix, const point3d<T>& opoint)
   {
      acme::quadix<T,3> quadix_;
   ...
   template <typename T>
   void fast_rotate(const trig_luts<T>& lut,
   ...
   template <typename T>
   point2d<T> fast_rotate(const trig_luts<T>& lut,
                                 const int rotation_angle,
                                 const point2d<T>& point)
   {
      point2d<T> point_;
   ...
   template <typename T>
   segment<T,2> fast_rotate(const trig_luts<T>& lut,
                                   const int rotation_angle,
                                   const segment<T,2>& segment)
   {
      acme::segment<T,2> segment_;
   ...
   template <typename T>
   triangle<T,2> fast_rotate(const trig_luts<T>& lut,
                                    const int rotation_angle,
                                    const triangle<T,2>& triangle)
   {
      acme::triangle<T,2> triangle_;
   ...
   template <typename T>
   quadix<T,2> fast_rotate(const trig_luts<T>& lut,
                                  const int rotation_angle,
                                  const quadix<T,2>& quadix)
   {
      acme::quadix<T,2> quadix_;
   ...
   template <typename T>
   point3d<T> fast_rotate(const trig_luts<T>& lut,
                                 const int rx, const int ry, const int rz,
                                 const point3d<T>& point)
   {
      point3d<T> point_;
   ...
   template <typename T>
   segment<T,3> fast_rotate(const trig_luts<T>& lut,
                                   const int rx, const int ry, const int rz,
                                   const segment<T,3>& segment)
   {
      acme::segment<T,3> segment_;
   ...
   template <typename T>
   triangle<T,3> fast_rotate(const trig_luts<T>& lut,
                                    const int rx, const int ry, const int rz,
                                    const triangle<T,3>& triangle)
   {
      acme::triangle<T,3> triangle_;
   ...
   template <typename T>
   quadix<T,3> fast_rotate(const trig_luts<T>& lut,
                                  const int rx, const int ry, const int rz,
   ...
   rectangle<T> aabb(const triangle<T,2>& triangle)
   {
      ...
      for (std::size_t i = 1; i < acme::triangle<T,2>::PointCount; ++i)
      {
   ...
   rectangle<T> aabb(const quadix<T,2>& quadix)
   {
      ...
      for (std::size_t i = 1; i < acme::quadix<T,2>::PointCount; ++i)
      {
   ...
   {
      acme::rectangle<T> rectangle_ = aabb(triangle);
   ...
   {
      acme::rectangle<T> rectangle_ = aabb(rectangle);
   ...
   {
      acme::rectangle<T> rectangle_ = aabb(quadix);
   ...
   {
      acme::rectangle<T> rectangle_ = aabb(circle);
   ...
   box<T,3> aabb(const triangle<T,3>& triangle)
   {
      ...
      for (std::size_t i = 1; i < acme::triangle<T,3>::PointCount; ++i)
      {
   ...
   template <typename T>
   box<T,3> aabb(const box<T,3>& box)
   {
      acme::box<T,3> box_;
   ...
   box<T,3> aabb(const quadix<T,3>& quadix)
   {
      ...
      for (std::size_t i = 1; i < acme::quadix<T,3>::PointCount; ++i)
      {
   ...
   template <typename T, typename OutputIterator>
   void generate_random_points(const T& x1, const T& y1,
                                      const T& x2, const T& y2,
   ...
   template <typename T, typename OutputIterator>
   void generate_random_points(const T& x1, const T& y1, const T& z1,
                                      const T& x2, const T& y2, const T& z2,
                                      const std::size_t& point_count,
                                      OutputIterator out)
   {
   ...
   {
      acme::box<T,3> region = make_box(x1,y1,z1,x2,y2,z2);
   ...
   template <typename T>
   vector2d<T> operator*(const vector2d<T>& v, const T& scale)
   {
      vector2d<T> v_;
   ...
   template <typename T>
   vector3d<T> operator*(const vector3d<T>& v, const T& scale)
   {
      vector3d<T> v_;
   ...
   template <typename T>
   vector2d<T> operator*(const T& scale, const vector2d<T>& v)
   {
      vector2d<T> v_;
   ...
   template <typename T>
   vector3d<T> operator*(const T& scale, const vector3d<T>& v)
   {
      vector3d<T> v_;
   ...
   {
      return acme::scale(scale,scale,point);
   }

   template <typename T>
   point3d<T> operator*(const point3d<T>& point, const T& scale)
   {
      return acme::scale(scale,scale,scale,point);
   }
   ...
   template <typename T>
   point3d<T> make_point(const point2d<T> point, const T& z)
   {
   ...
   template <typename T>
   vector3d<T> make_vector(const vector2d<T> v, const T& z)
   {
   ...
   template <typename T>
   triangle<T,2> make_triangle(const T& x1, const T& y1,
                                      const T& x2, const T& y2,
                                      const T& x3, const T& y3)
   {
      triangle<T,2> triangle_;
   ...
   template <typename T>
   triangle<T,3> make_triangle(const T& x1, const T& y1, const T& z1,
                                      const T& x2, const T& y2, const T& z2,
                                      const T& x3, const T& y3, const T& z3)
   {
   ...
   template <typename T>
   quadix<T,2> make_quadix(const T& x1, const T& y1,
                                  const T& x2, const T& y2,
                                  const T& x3, const T& y3,
                                  const T& x4, const T& y4)
   {
   ...
   template <typename T>
   quadix<T,3> make_quadix(const T& x1, const T& y1, const T& z1,
                                  const T& x2, const T& y2, const T& z2,
                                  const T& x3, const T& y3, const T& z3,
                                  const T& x4, const T& y4, const T& z4)
   {
   ...
   template <typename T>
   plane<T,3> make_plane(const T& x1, const T& y1, const T& z1,
                                const T& x2, const T& y2, const T& z2,
                                const T& x3, const T& y3, const T& z3)
   {
      plane<T,3> plane_;
   ...
   template <typename T>
   plane<T,3> make_plane(const T& px, const T& py, const T& pz,
   ...
   template <typename T>
   plane<T,3> make_plane(const point3d<T>& point1,
                                const point3d<T>& point2,
                                const point3d<T>& point3)
   {
      return make_plane
   ...
   template <typename T>
   plane<T,3> make_plane(const point3d<T>& point,
